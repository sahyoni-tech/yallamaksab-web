...
export function PendingList() {
  ...
  return (
    <div className="container" style={{ paddingBlock: 32 }}>
      ...
      {rows.map((m) => (
        <div key={m.id} style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, padding: 16, marginBlock: 8, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div>
            <strong>{m.business_name}</strong>
            <div style={{ color: "var(--muted)", fontSize: 14 }}>{m.category}</div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button className="btn-primary" disabled={busyId === m.id} onClick={() => act(m.id, (id) => approveMerchant(supabase, id))}>{ar.adminApprove}</button>
            <button className="btn-secondary" disabled={busyId === m.id} onClick={() => act(m.id, (id) => rejectMerchant(supabase, id))}>{ar.adminReject}</button>
          </div>
        </div>
      ))}
    </div>
