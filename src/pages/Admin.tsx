...
export function Admin() {
  ...
  if (!isAdmin) {
    return (
      <div className="container" style={{ paddingBlock: 48, display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 16 }}>
        <p>{ar.adminNotAuthorized}</p>
        <button className="btn-secondary" onClick={signOut}>{ar.adminSignOut}</button>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="container" style={{ display: "flex", gap: 8, paddingTop: 24, alignItems: "center" }}>
        ...
        <button className="btn-secondary" onClick={signOut} style={{ marginInlineStart: "auto" }}>{ar.adminSignOut}</button>
      </div>
